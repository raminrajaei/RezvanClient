using ATA.HR.Client.Web.Components;
...
public class NotificationService : INotificationService
{
    ...
    public async ValueTask AlertAsync(NotificationType notificationType, string message, int waitSeconds = AppConstants.AlertDefaultTimeout, AlertifyPosition position = AlertifyPosition.TopCenter)
    {
        // Set position
        await _jsRuntime.InvokeVoidAsync("alertify.set", "notifier", "position", position.ToEnumDisplayName()!);

        // Show alert
        string jsIdentifier = notificationType switch
        {
    ...
    public void Toast(NotificationType notificationType, string message, int waitSeconds = 5, bool showProgressBar = true)
    {
        // Toast Params
        ...
        toastParameters.Add(nameof(toastComponent.NotificationType), notificationType);
