...
namespace Bit.Http.Contracts
{
    ...
    public class BitODataResponse<T>
    {
...
namespace Bit.Http.Contracts
{
    ...
    public interface ITokenProvider
    {
        ...
        Task SetBitTokenAsync(Token? token);
...
namespace Bit.Http.Implementations
{
    public class DefaultSecurityService : ISecurityService
    {
        ...
        private readonly ICoreAPIs _coreAPIs;

        public DefaultSecurityService(HttpClient httpClient, ITokenProvider tokenProvider, ICoreAPIs coreAPIs)
        {
        ...
        public virtual async Task<Token> LoginWithCredentials(string userName, string password, string client_id, string client_secret, string[]? scopes = null, IDictionary<string, string?>? acr_values = null, CancellationToken cancellationToken = default)
        {
            ...
            Token bitToken = await DefaultJsonContentFormatter.Current.DeserializeAsync<Token>(responseContent, cancellationToken).ConfigureAwait(false);

            bitToken.login_date = DateTimeOffset.UtcNow;

            bitToken.expires_in = (int)AppConstants.AuthToken.Lifetime.TotalSeconds;

            bitToken.expires_at = (DateTimeOffset.UtcNow + AppConstants.AuthToken.Lifetime).Ticks; //Utc 

            await _tokenProvider.SetBitTokenAsync(bitToken).ConfigureAwait(false);

            await LoginInATACore(userName, password);

            return bitToken;
        }

        public async Task LoginInATACore(string userName, string password)
        {
            var token = await _coreAPIs.Auth_Login(new ATALoginDto
            {
                SSOToken = userName,
                AppName = password
            });

            await _tokenProvider.SetJwtTokenAsync(token.AccessToken).ConfigureAwait(false);
        }
    }
    ...
    public class DefaultTokenProvider : ITokenProvider
    {
        ...
        public async Task SetBitTokenAsync(Token? token)
        {
            if (token != null)
            {
        ...
        public async Task SetJwtTokenAsync(string jwtToken)
        {
            if (jwtToken != null)
            {
                await _localStorageService.SetItemAsStringAsync(AppConstants.AuthToken.JwtTokenLocalStorageKey, jwtToken);
            }

            else
            {
                await _localStorageService.RemoveItemAsync(AppConstants.AuthToken.JwtTokenLocalStorageKey);
            }
        }
    }
        ...
        where TDto : IDto
    {
        ...
        public virtual async Task<T> DeserializeAsync<T>(Stream responseStream, ODataContext? context, CancellationToken cancellationToken)
        {
            ...
            BitODataResponse<T> bitODataResponse = await json.ToObjectAsync<BitODataResponse<T>>().ConfigureAwait(false);

            if (context != null)
                context.TotalCount = bitODataResponse.TotalCount;

            return bitODataResponse.Value;
        }
    }
}
