...
namespace ATA.HR.Client.Web.Extensions
{
    public static class AuthStateTaskExtensions
    {
        ...
        public static async Task<string> GetUserUnit(this Task<AuthenticationState> authStateTask)
        {
            var authState = await authStateTask;

            return authState.User.Claims
                .Where(c => c.Type.Equals(nameof(ATATokenClaims.UnitName), StringComparison.OrdinalIgnoreCase))
                .Select(c => c.Value)
                .FirstOrDefault();
        }
        ...
        public static async Task<bool> IsAdminUser(this Task<AuthenticationState> authStateTask)
        {
            var authState = await authStateTask;
            ...
            return authState.User.Claims
                .Where(c => c.Type == ClaimTypes.Role)
                .Select(c => c.Value)
                .Any(c => c == AppConstants.Roles.Administrator);
        }
        ...
        public static async Task<bool> HasUserClaim(this Task<AuthenticationState> authStateTask, string claim)
        {
            var claims = await authStateTask.GetUserClaims();
